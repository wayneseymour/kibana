...
export const tap = (log: ToolingLog) => (x: string) => {
  log.info(fx(x));
...
const logLoud = (x: number): void => {
  const brightAndNoticeable = [
    `\t${chalk.bold.cyanBright.underline('Looping')} `,
...
export async function printInfoAndInitOutputLogging(
  ...
  console.log('\nTest Loop for Measuring Es Archiver Perf');
  logLoud(loopCount);

  log.info(chalk.bold.cyanBright.underline(JSON.stringify(archives, null, 2)));
  log.info(chalk.bold.cyanBright.underline(hardware()));
