...
_guard() {
  ...
  if [ "${#failures[@]}" -eq 0 ]; then
    local zipped=$(is_zipped "$archive")
    if [[ "$zipped" != "" ]]; then
      unzip_and_keep "$archive"
    fi

    local containsDotKibanaRefs=$(grep -is '"index": ".kibana' "$archiveJson")
...
should_migrate() {
  ...
  if [[ "$guardResults" == "" ]]; then
    ...
    if [ "${intersectionsLength}" -gt 0 ]; then
      local foundUsages=($(usages_list "$archive"))
      local configs=($(find_configs "${foundUsages[@]}"))
